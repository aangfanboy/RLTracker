...
class NUTSKernelResults(mcmc_util.PrettyNamedTupleMixin, collections.namedtuple('NUTSKernelResults', ['target_log_prob', 'grads_target_log_prob', 'step_size', 'log_accept_ratio', 'leapfrogs_taken', 'is_accepted', 'reach_max_depth', 'has_divergence', 'energy', 'seed'])):
  """Internal state and diagnostics for No-U-Turn Sampler."""
  __slots__ = ...


class MomentumStateSwap(mcmc_util.PrettyNamedTupleMixin, collections.namedtuple('MomentumStateSwap', ['momentum_swap', 'state_swap'])):
  """Internal state and diagnostics for No-U-Turn Sampler."""
  __slots__ = ...
...
class TreeDoublingState(mcmc_util.PrettyNamedTupleMixin, collections.namedtuple('TreeDoublingState', ['momentum', 'state', 'target', 'target_grad_parts'])):
  """Internal state and diagnostics for No-U-Turn Sampler."""
  __slots__ = ...
...
class NoUTurnSampler(kernel.TransitionKernel):
  ...
  _arXiv preprint arXiv:1701.02434_, 2018. https://arxiv.org/abs/1701.02434
  """
  def __init__(self, target_log_prob_fn, step_size, max_tree_depth=..., max_energy_diff=..., unrolled_leapfrog_steps=..., parallel_iterations=..., experimental_shard_axis_names=..., name=...) -> None:
  ...
  def one_step(self, current_state, previous_kernel_results, seed=...): # -> tuple[Any, NUTSKernelResults]:
    ...
  
  def init_momentum_state_memory(self, input_tensors): # -> list[Any]:
...
def has_not_u_turn_at_all_index(read_indexes, direction, momentum_state_memory, momentum_right, state_right, no_u_turns_within_tree, log_prob_rank, shard_axis_names=...):
  """Check u turn for early stopping."""
  ...

def has_not_u_turn(state_diff, momentum_left, momentum_right, log_prob_rank, shard_axis_names=...): # -> bool:
...
def compute_hamiltonian(target_log_prob, momentum_parts, shard_axis_names=...):
